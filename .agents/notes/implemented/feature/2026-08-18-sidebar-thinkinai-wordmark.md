...
`ThinkInAIWordmark` is a package-internal SVG in `ui-sidebar`: a check-circle drawn from the product mark plus the ThinkInAI name, on the DeepSeek wordmark's 182×24 viewBox. Fills and strokes use `currentColor`, so the mark inherits the sidebar's `--dsw-alias-label-primary` with `BrandWordmark`. There is no hardcoded palette and no neon glow, because a glow plate would only read in dark Appearance.

`SidebarRoot` stacks the two marks in the existing brand New Session button. The expanded logo row grows to 72px and top-aligns the collapse toggle beside the DeepSeek mark. The collapsed rail still shows only the whale and the panel icon.
