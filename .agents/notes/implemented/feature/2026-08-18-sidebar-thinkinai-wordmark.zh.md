...
`ThinkInAIWordmark` 是 `ui-sidebar` 的包内 SVG：从产品标识抽出的圆圈对勾，加上 ThinkInAI 名称，画在 182×24 viewBox 上。描边与填充使用 `currentColor`，因此它与官方 slot 品牌一样继承侧栏的 `--dsw-alias-label-primary`。没有写死的色板，也没有霓虹光晕，因为光晕底板只在深色 Appearance 下可读。

`SidebarRoot` 保留官方的 `sidebar.brand.mark` / `sidebar.brand.name` 行，并把 ThinkInAI 长条叠在现有品牌 New Session 按钮里。展开时 logo 行以 `min-height: 72px` 增高，折叠按钮与官方品牌行顶对齐。折叠轨道仍然只显示 mark slot 和面板图标。
